...
/**
 * @author tag
 * @version $Id: Polyline.java 3236 2007-10-09 11:10:27Z patrickmurris $
 */
...
//    public final static int LOXODROME = RHUMB_LINE;
    public final int ANTIALIAS_DONT_CARE = GL.GL_DONT_CARE;
    public final int ANTIALIAS_FASTEST = GL.GL_FASTEST;
    public final int ANTIALIAS_NICEST = GL.GL_NICEST;
    ...
    /** Returns the length of the line as drawn. If the path follows the terrain, the length returned is the
     * distance one would travel if on the surface. If the path does not follow the terrain, the length returned is
     * the distance along the full length of the path at the path's elevations and current path type.
    ...
    /**
     * Sets the stipple pattern for specifying line types other than solid. See the OpenGL specification or
     * programming guides for a description of this parameter. Stipple is also affected by the path's
     * stipple factor, {@link #setStippleFactor(int)}.
    ...
    /**
     * Sets the stipple factor for specifying line types other than solid. See the OpenGL specification or
     * programming guides for a description of this parameter. Stipple is also affected by the path's
     * stipple pattern, {@link #setStipplePattern(short)}.
    ...
    /**
     * Sets the paths positions as latitude and longitude values at a constant elevation.
    ...
    private boolean isSegmentVisible(DrawContext dc, Position posA, Position posB)
    {
        ...
        Cylinder cyl = new Cylinder(ptA, ptB, r);
