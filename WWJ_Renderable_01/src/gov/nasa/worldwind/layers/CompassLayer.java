...
/**
 * CompassLayer with possibility of needle rolling inside.
 * 
 * @author tag
 * @version $Id$
 */
public class CompassLayer extends AbstractLayer
{
    ...
    private final static String NOTCHED_COMPASS_IMAGE = "images/notched-compass.png"; // TODO: make configurable
    private final static String NEEDLE_COMPASS_IMAGE = "images/needlecompass.png";
    private final static String NEEDLE_IMAGE = "images/needle.png";
    private String iconFilePath = NOTCHED_COMPASS_IMAGE;
    private String needleFilePath = NEEDLE_IMAGE;
    ...
    public CompassLayer(boolean needle)
    {
        this.needle = needle;
        if (needle)
        {
            this.setIconFilePath(NEEDLE_COMPASS_IMAGE);
        }
        else
        {
            this.setIconFilePath(NOTCHED_COMPASS_IMAGE);
        }
        this.setOpacity(0.8); // TODO: make configurable
    }
    ...
    public CompassLayer(String iconFilePath, boolean needle)
    {
        this.needle = needle;
        this.setIconFilePath(iconFilePath);
        this.setOpacity(0.8); // TODO: make configurable
    ...
    public CompassLayer(String iconFilePath, String needleFilePath)
    {
        this.needle = true;
        this.setIconFilePath(iconFilePath);
        this.setNeedleFilePath(needleFilePath);
        this.setOpacity(0.8); // TODO: make configurable
    }
    ...
    public String getNeedleFilePath()
    {
        return needleFilePath;
    }

    public void setNeedleFilePath(String needleFilePath)
    {
        if (needleFilePath == null)
        {
            String message = Logging.getMessage("nullValue.IconFilePath");
            Logging.logger().severe(message);
            throw new IllegalArgumentException(message);
        }
        this.needleFilePath = needleFilePath;
    }
    ...
    private void drawCompass(DrawContext dc)
    {
        ...
        try
        {
            Texture iconTexture = dc.getTextureCache().get(this.iconFilePath);
            if (iconTexture == null)
            {
                this.initializeTexture(dc);
                iconTexture = dc.getTextureCache().get(this.iconFilePath);
                if (iconTexture == null)
                {
                    // TODO: log warning
                    return;
            ...
            if (!this.needle && this.showHeading)
            {
    ...
    private void drawNeedle(DrawContext dc)
    {
        if (this.needleFilePath == null)
        {
            return;
        }
        ...
        try
        {
            Texture iconTexture = dc.getTextureCache().get(this.needleFilePath);
            if (iconTexture == null)
            {
                this.initializeNeedleTexture(dc);
                iconTexture = dc.getTextureCache().get(this.needleFilePath);
                if (iconTexture == null)
                {
                    // TODO: log warning
                    return;
    ...
    private void initializeTexture(DrawContext dc)
    {
        ...
        try
        {
            ...
            dc.getTextureCache().put(this.iconFilePath, iconTexture);
        }
        catch (IOException e)
        {
    ...
    private void initializeNeedleTexture(DrawContext dc)
    {
        Texture iconTexture = dc.getTextureCache().get(this);
        if (iconTexture != null)
        {
        ...
        try
        {
            InputStream iconStream = this.getClass().getResourceAsStream("/" + this.needleFilePath);
            if (iconStream == null)
            {
                File iconFile = new File(this.needleFilePath);
                if (iconFile.exists())
                {
                    iconStream = new FileInputStream(iconFile);
                }
            ...
            this.iconWidth = iconTexture.getWidth();
            this.iconHeight = iconTexture.getHeight();
            dc.getTextureCache().put(this.needleFilePath, iconTexture);
        }
        catch (IOException e)
        {
            String msg = Logging.getMessage("layers.IOExceptionDuringInitialization");
