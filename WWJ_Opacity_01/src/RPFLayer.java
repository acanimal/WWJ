...
//    }
    ...
    private static class BasicSurfaceTile implements Disposable, Cacheable, SurfaceTile
    {
        ...
        public Extent getExtent(DrawContext dc)
        {
            ...
            {
                this.extent =
                    Sector.computeBoundingCylinder(dc.getGlobe(), dc.getVerticalExaggeration(), this.getSector());
        ...
        public Retriever call() throws Exception
        {
            ...
            try
            {
                ...
                if (!this.interrupted())
                {
                    ...
                    if (file == null)
                    {
                        String message = Logging.getMessage("generic.CantCreateCacheFile", cacheFilePath)
                            + cacheFilePath;
                        throw new IOException(message);
                    }
...
//            return;
    ...
    private void sendRequests()
    {
        ...
        {
            FrameRequest request;
            while (!WorldWind.getRetrievalService().isFull() && (request = this.downloadQueue.poll()) != null)
            {
                if (!request.isStale())
